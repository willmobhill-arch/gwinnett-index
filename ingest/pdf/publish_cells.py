...
VERIFIED_AGAINST_RENDER = {
    ...
    ('4-B', 139): 'Every cell compared against rendered pp139-141 on 2026-08-17. Ten '
                  'cells in the parking-spaces column had stacked values glued into '
                  'one number by the superscript fold ("20" over "18" stored as '
                  '"1820"); the extractor was fixed and all ten re-checked against '
                  'the render. Stacked values pair line-by-line with the "Required '
                  'for Each" lines beside them. Rows 0, 9, 38 and 44 are section '
                  'bands (a. RESIDENTIAL etc.); row 23 "14. Shopping Centers:" is a '
                  'sub-heading whose (a)-(e) children are subordinate only by '
                  'numbering. Superscript footnote markers flatten to trailing '
                  'digits ("sf1", "GFA2", "GLA3" -- the footnote texts are printed '
                  'on p140). "1000 sf" without a comma in the Daycare row and the '
                  '"400,000-599,000" range are the source\'s own inconsistencies, '
                  'reproduced as printed.',
    ('2-C', 56): 'Every cell compared against rendered pp56-69 on 2026-08-17, in '
                 'three independent passes (rows 0-122, 123-250, 251-332), with '
                 'single-letter district values checked against the same-page '
                 'header column by column. Zero discrepancies. Rows 0, 3, 22, 39, '
                 '129, 155, 217, 279 and 317 are full-width shaded section bands; '
                 'about forty further rows are italic sub-group headings whose '
                 'label spans the value columns in print but which are stored as '
                 'ordinary rows with empty value cells -- indistinguishable here '
                 'from a use with no permissions, so read an all-blank row as '
                 'possibly a heading. "Blook and Organ Banks", "Stationary '
                 'Stores", "Fastner" and NAICS "45382" are the source\'s own '
                 'errors, reproduced as printed.',
    ('2-C', 70): 'Every cell compared against rendered pp70-84 on 2026-08-17, in '
                 'three independent passes (rows 0-111, 112-229, 230-331), with '
                 'single-letter district values checked against the same-page '
                 'header column by column. Zero discrepancies. Same band/heading '
                 'caveat as the residential half: full-width shaded bands plus '
                 'NAICS-coded italic sub-headings are stored as ordinary rows with '
                 'empty value cells. WARNING on row 312: the print has TWO ruled '
                 'rows (5178 Freestanding Towers and Antennae / 5178 Additions to '
                 'Existing Towers (Colocation)) sharing one merged cell "SEE '
                 'ARTICLE 3, SECTION 342" that spans ALL district columns and both '
                 'rows; the store folds them into one row with the merged text in '
                 'the CBD column only, so the cross-reference applies to every '
                 'district and both entries even though it is stored in one cell. '
                 'Row 111 (5617) is typeset like a band but carries values and is '
                 'a data row.',
    ('2-D', 85): 'Every cell compared against rendered pp85-88 on 2026-08-17, '
                 'including every blank, with letter-bearing rows checked against '
                 'the header x-positions. Zero discrepancies. Rows 0, 1, 9, 24 and '
                 '44 are section bands; rows 3, 11, 16, 46 and 51 are bold-italic '
                 'sub-headings in the USES column, and the subordination of the '
                 'rows beneath them (e.g. "Cattle" under "For Non-Commercial '
                 'Purposes:") is positional only. "Accessory Day Car Centers", '
                 '"Swamp Meets" and "personage" are the source\'s own typos, '
                 'reproduced as printed; "Horse Stable, Personal" appears twice '
                 'because it does in the ordinance. The A/S legend printed above '
                 'the table is not part of the stored rows.',
    ('2-D', 89): 'Every cell compared against rendered pp89-93 on 2026-08-17, '
                 'including every blank, with single-letter placements counted '
                 'from the same-page header. Zero discrepancies. page_to corrected '
                 '94 -> 93: p94 carries no table (Article Three starts there). '
                 'Same structure as the residential half: bands at rows 0, 1, 9, '
                 '24, 44; bold-italic sub-headings at 3, 11, 16, 46, 51 with '
                 'position-only subordination; the same source typos reproduced as '
                 'printed.',
