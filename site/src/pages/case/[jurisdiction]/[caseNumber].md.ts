...
import { acres, date, zone, rawCode, isMined, decisionWithAction } from '../../../lib/format';
...
export const GET: APIRoute = ({ props }) => {
  ...
  const body = [
    ...
    facts([
      ...
      ['Decision', c.motion_action ? decisionWithAction(c.decision, c.motion_action) : rawCode(c.decision)],
      ['Decision date', date(c.decision_date)],
      ['Moved by', c.moved_by], ['Seconded by', c.seconded_by],
      ['Status', rawCode(c.status)], ['Heard by', c.hearing_body],
      ['Voted for', c.voted_for], ['Voted against', c.voted_against],
    ]),
    '',
    c.motion_action && c.motion_action !== 'approve'
      ? `> **This is not an approval.** The motion that carried was a motion to ` +
        `${c.motion_action}. Read alone, "${c.decision}" against a land-use case reads ` +
        'as approved.\n'
      : null,
    ...
    facts([
      ...
      ['Decision recorded in', c.decision_source_url !== c.source_url ? c.decision_source_url : null],
